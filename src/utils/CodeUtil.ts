import { codeFileTypes } from '../constants/FileTypeConstatnts.ts'
...
export function isCode(filePath: string): boolean {
  ...
  return codeFileTypes[ext] !== undefined || false
}
...
export function getCodeLanguage(filePath: string): string {
  const ext = filePath.split('.').pop() || ''
  return codeFileTypes[ext] !== undefined ? ext : ''
}
 ...
 * @param filePath 文件路径
 */
export function getCodeType(filePath: string): string {
  const ext = filePath.split('.').pop() || ''
  return codeFileTypes[ext] || ''
}
