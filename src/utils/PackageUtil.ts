 ...
 * @param filePath 文件路径
 */
export function isPackage(filePath: string): boolean {
 ...
 */
export function convertChildren(packageInfos: PackageInfo[]): PackageTreeOption[] {
  ...
  for (const info of packageInfos) {
    if (info.dir) {
      ...
      const name = info.path.endsWith('/')
        ? info.path.substring(0, info.path.lastIndexOf('/'))
        : info.path
 ...
 * @param path 文件夹路径
 */
function addPathToTree(tree: PackageTreeOption[], path: string): void {
...
function addToDirectory(
  tree: PackageTreeOption[],
  dirPath: string,
  fileNode: PackageTreeOption
): void {
 ...
 * @param nodes 文件夹节点
 */
function updateAllFolderCounts(nodes: PackageTreeOption[]): void {
  nodes.forEach(node => {
    if (node.children) {
      // 计算该文件夹下的总文件数
 ...
 * @param nodes 文件夹节点
 */
function sortTreeNodes(nodes: PackageTreeOption[]): void {
  nodes.sort((a, b) => {
    // 如果都是文件夹或都是文件，按日期排序
 ...
 * @param nodes 文件夹节点
 */
function calculateFileCount(nodes: PackageTreeOption[]): number {
  let count = 0
  nodes.forEach(node => {
    if (node.children) {
      // 是文件夹，递归计算子节点
