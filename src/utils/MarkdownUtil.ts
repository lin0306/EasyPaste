 ...
 */
export function getRenderer(bashPath: string | undefined = undefined): Renderer {
  ...
  renderer.image = (info: Tokens.Image) => {
    ...
    if (bashPath && (info.href.startsWith('./') || info.href.startsWith('../'))) {
      ...
      if (info.href.startsWith('./')) {
        ...
        const { url, path } = removeRelativePath(
          info.href.substring(3),
