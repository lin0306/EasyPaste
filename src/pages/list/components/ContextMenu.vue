 ...
 * @param event 事件对象
 */
const calculateMenuPosition = (event: any): void => {
  // 计算菜单位置
  let x = event.pageX
  let y = event.pageY
  setTimeout(() => {
 ...
 * @param event 事件对象
 */
const handleContextMenuOutside = (event: any): void => {
 ...
 * @param itemId 项目id
 */
const onOpenTextEditorWindow = (itemId: number): void => {
  openItemEditWindow(itemId)
}
...
<template>
  ...
  >
    <div class="context-menu-item" @click="onCopy(props.item, message)">
      <span>{{ currentLanguage.pages.list.contextMenu.copy }}</span>
    </div>
    <div
      v-if="props.item && !props.item.is_topped"
      class="context-menu-item"
      @click="onTop(props.item.id, props.index)"
    >
      <span>{{ currentLanguage.pages.list.contextMenu.onTop }}</span>
    </div>
    <div
      v-if="props.item && props.item.is_topped"
      class="context-menu-item"
    ...
    >
      <span>{{ currentLanguage.pages.list.contextMenu.unTop }}</span>
    </div>

    <!-- 单击单个文件 -->
    <template v-if="props.menuType === 'file' && filePath">
      ...
      >
        <span>{{ currentLanguage.pages.list.contextMenu.copyFile }}</span>
      </div>
    </template>
    ...
    <template v-if="props.menuType === 'link'">
      <div class="context-menu-divider"></div>
      <div class="context-menu-item" @click="openLink(props.item.content)">
        <span>{{ currentLanguage.pages.list.contextMenu.openLink }}</span>
      </div>
    </template>
    ...
    >
      <span>{{ currentLanguage.pages.list.contextMenu.delete }}</span>
    </div>
  </div>
</template>
