...
// 是否自动跳转到最新数据
const isAutoGoToLatestData = ref(false)

// 是否显示清空剪贴板的弹窗状态
export const showClearDataModal = ref(false)

// 是否显示详细时间
export const displayDetailTime = ref(false)

/**
 * 加载剪贴板项目列表
 * @param reset 是否重新获取数据
 */
export const loadClipboardItems = async (reset: boolean = true): Promise<void> => {
  if (reset) {
    // 重置列表和分页状态
  ...
  try {
    ...
    scrollState.hasMore =
      items.length >= scrollState.pageSize && clipboardItems.value.length < total

    // 预加载图片的base64数据
    for (const item of items) {
      if (item.type === 'file') {
        checkFilesExist(item.file_path)
      }
      if (item.type === 'image') {
 ...
 * @param item 需要插入的内容
 */
export const insertClipboardItem = async (item: ClipboardItem): Promise<void> => {
  console.log('开始插入数据到列表:', item)
  // 根据查询条件判断是否可以插入
 ...
 * @param index 项目索引
 */
export const onTop = async (id: number, index: number): Promise<void> => {
 ...
 * @param index 项目索引
 */
export const onUnTop = async (id: number, index: number): Promise<void> => {
  selectedItemId.value = id
  const db = await ClipboardDBService.getInstance()
  ...
  const item = await db.getItem(id)
  if (item) {
    if (clipboardItems.value && clipboardItems.value.length === 1) {
      clipboardItems.value[0] = { ...item }
      return
    }
    let topItems = 0
    ...
    if (item) {
      for (let i = 0; i < clipboardItems.value.length; i++) {
        const currentItem = clipboardItems.value[i]
        if (currentItem.is_topped) {
          topItems++
          continue
        }
        if (currentItem.copy_time < item.copy_time) {
          if (i === 0 || (topItems === 1 && i === 1)) {
            // 置顶前，置顶后都是第一条数据，直接修改内容
            clipboardItems.value[0] = { ...item }
          } else {
            clipboardItems.value.splice(index, 1)
            clipboardItems.value.splice(i - 1, 0, { ...item })
          }
          isUnTop = true
          break
        }
...
export const bindTag = async (
  ...
  message.success(currentLanguage.value.pages.list.bindSuccessMsg)
...
export const removeItemTag = async (
  ...
  // 删除标签
  clipboardItems.value[index].tags.splice(
    clipboardItems.value[index].tags.findIndex(t => t.id === tag.id),
    1
  )
  message.success(language.pages.list.unBindSuccessMsg)
  await emit('update-bind-quantity')
  setTimeout(() => {
    selectedItemId.value = undefined
  }, 20)
...
export const removeItem = async (
  ...
  if (item.type === 'file') {
    if (item.file_path.startsWith('[')) {
      JSON.parse(item.file_path).forEach((path: string) => {
        removeCache(path)
      })
    } else {
      removeCache(item.file_path)
    }
  }
  message.success(language.pages.list.deleteSuccessMsg)
  // 如果列表数据数量小于9，加载新数据
  if (clipboardItems.value.length <= 9) {
    await loadClipboardItems(false)
  }
  setTimeout(() => {
    selectedItemId.value = undefined
 ...
 * @param message 消息
 */
export const onCopy = async (item: ClipboardItem, message: MessageApiInjection): Promise<void> => {
  setTimeout(async () => {
 ...
 * @param tagId 标签ID
 */
export const isItemTagged = (itemId: number, tagId: number): boolean => {
 ...
 * @param message 消息提示
 */
export const clearAllData = async (message: MessageApiInjection): Promise<void> => {
  // 清空历史记录
  try {
    const db = await ClipboardDBService.getInstance()
...
async function initUpdateDataHistoryRestrictListener(): Promise<UnlistenFn> {
...
/**
 * 初始化更新数据是否跳转到最新的数据位置监听
 */
let updateAutoGoToLatestDataListener: any = null

async function initUpdateAutoGoToLatestDataListener(): Promise<UnlistenFn> {
  return await listen('update-auto-go-to-latest-data', async (event: any) => {
    isAutoGoToLatestData.value = event.payload.isGoToLatestData
  })
}
 ...
 * 初始化文本内容更新监听
 */
let updateContentListener: any = null

async function initUpdateContentListener(): Promise<UnlistenFn> {
  return await listen('update-content', async (event: any) => {
    const itemId = parseInt(event.payload.itemId)
    console.log('更新内容', itemId)
    const db = await ClipboardDBService.getInstance()
    const item = await db.getItem(itemId)
    console.log('更新内容', item)
    if (item) {
      clipboardItems.value.forEach(item1 => {
        if (item1.id === itemId) {
 ...
 * 初始化数据重新加载监听
 */
let reloadItemsListener: any = null
...
async function initDisplayDetailTimeListener(): Promise<UnlistenFn> {
  return await listen('update-display-detail-time', async (event: any) => {
    displayDetailTime.value = event.payload.displayDetailTime
  })
}
 ...
 * 初始化剪贴板数据配置
 */
export async function initializeClipboardData(): Promise<void> {
  try {
    loadClipboardItems(true).catch(e => {
      console.error('剪贴板列表数据加载失败', e)
    })
    // 启动剪贴板监听服务
    ...
    if ((await getDataRetentionDays()) > 0) {
      DataClearService.getInstance().then(clearTimer => {
        clearTimer.startDataClear()
      })
    }

    // 添加更新数据是否跳转到最新的数据位置事件监听
    updateAutoGoToLatestDataListener = await initUpdateAutoGoToLatestDataListener()

    // 获取自动跳转到最新的数据位置
    isAutoGoToLatestData.value = await getAutoGoToLatestData()

    // 添加更新数据内容事件监听
    updateContentListener = await initUpdateContentListener()

    // 添加数据重新加载事件监听
    reloadItemsListener = await initReloadItemsListener()
 ...
 * 销毁剪贴板数据配置
 */
export async function destroyClipboardData(): Promise<void> {
  // 清除剪贴板监听服务
  ...
  // 销毁监听修改自动跳转最新数据事件
  if (updateAutoGoToLatestDataListener) {
    updateAutoGoToLatestDataListener()
  }

  // 销毁监听修改数据内容事件
  if (updateContentListener) {
    updateContentListener()
  }

  // 销毁监听数据重新加载事件
  if (reloadItemsListener) {
    reloadItemsListener()
  }
