import { reactive, ref } from 'vue'
import ClipboardDBService from '../../../services/ClipboardDBService.ts'
import { loadClipboardItems } from './ClipboardDataComposable.ts'
import { error } from '@tauri-apps/plugin-log'
import { listen, UnlistenFn } from '@tauri-apps/api/event'
import {
  getBindTagBtnShowLocation,
  getEnableTag,
  getTagListLocation,
} from '../../../store/Settings.ts'
import type { MessageApiInjection } from 'naive-ui/es/message/src/MessageProvider'
import { currentLanguage } from '../../../services/LanguageService.ts'

/**
 * 标签列表
 */
export const TagItems = ref<TagItem[]>()

/**
 * 标签选中状态
 */
export const selectedTagState = reactive({
  selectedTagId: undefined as number | undefined,
})

/**
 * 标签设置
 */
export const tagSettingState = reactive({
  isShow: false,
  location: '',
  listLocation: '',
})

/**
 * 加载所有标签
 */
export const loadTags = async () => {
  const db = await ClipboardDBService.getInstance()
  TagItems.value = await db.getAllTags()
}

/**
 * 处理标签点击事件
 * @param {number} tagId - 标签ID
 */
export const handleTagClick = (tagId: number): void => {
  // 如果点击的是当前已选中的标签，则取消选中
  if (selectedTagState.selectedTagId === tagId) {
    selectedTagState.selectedTagId = undefined
  } else {
    // 否则选中该标签
    selectedTagState.selectedTagId = tagId
  }

  // 根据选中的标签过滤剪贴板列表（重置列表）
  loadClipboardItems(true)
}

/**
 * 加载标签设置状态
 */
export const loadTagSettingState = async (): Promise<void> => {
  tagSettingState.isShow = await getEnableTag()
  tagSettingState.location = await getBindTagBtnShowLocation()
  tagSettingState.listLocation = await getTagListLocation()
}

/**
 * 初始化加载标签事件监听
 */
let loadTagsUnListener: any = null

async function initLoadTagsListener(): Promise<UnlistenFn> {
  return await listen('reload-tags', async (_event: any) => {
    await loadTags()
    // todo 暂时先刷新列表重新取数据，避免标签删除了，但是列表数据没更新
    await loadClipboardItems(true)
  })
}

/**
 * 初始化更新标签设置状态监听
 */
let updateTagSettingStateListener: any = null

async function initUpdateTagSettingStateListener(): Promise<UnlistenFn> {
  return await listen('update-tag-setting-state', async (event: any) => {
    tagSettingState.isShow = event.payload.isShow
    tagSettingState.location = event.payload.location
  })
}

/**
 * 初始化更新标签列表展示位置设置状态监听
 */
let updateTagListLocationListener: any = null

async function initUpdateTagListLocationListener(): Promise<UnlistenFn> {
  return await listen('update-tag-list-location', async (event: any) => {
    tagSettingState.listLocation = event.payload.location
  })
}

/**
 * 初始化标签配置
 */
export const initializeTag = async (message: MessageApiInjection): Promise<void> => {
  try {
    // 加载标签列表
    loadTags().catch(e => {
      message.error(currentLanguage.value.pages.list.tagLoadFailed)
      error('加载标签列表失败' + e)
    })

    // 加载标签设置状态
    loadTagSettingState().catch(e => {
      error('加载标签设置状态失败' + e)
    })

    // 添加更新标签设置状态事件监听
    updateTagSettingStateListener = await initUpdateTagSettingStateListener()

    // 添加加载标签事件监听
    loadTagsUnListener = await initLoadTagsListener()

    updateTagListLocationListener = await initUpdateTagListLocationListener()
  } catch (e) {
    console.error('标签配置初始化失败', e)
    error('标签配置初始化失败' + e)
  }
}

/**
 * 销毁标签配置
 */
export const destroyTag = (): void => {
  // 清除加载标签事件监听
  if (loadTagsUnListener) {
    loadTagsUnListener()
  }
  // 清除更新标签设置状态事件监听
  if (updateTagSettingStateListener) {
    updateTagSettingStateListener()
  }
  // 清除更新标签列表位置事件监听
  if (updateTagListLocationListener) {
    updateTagListLocationListener()
  }
}
