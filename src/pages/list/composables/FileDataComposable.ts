import { exists, readDir } from '@tauri-apps/plugin-fs'
import { ref } from 'vue'
import { initSettings } from '../../../store/Settings.ts'
import { initShortcutKeys } from '../../../store/ShortcutKeys.ts'
import { filePathConvertFileName } from '../../../utils/TextUtil.ts'
import { listen, UnlistenFn } from '@tauri-apps/api/event'
import { info } from '@tauri-apps/plugin-log'

/**
 * 文件是否存在缓存，用于存储文件是否存在状态
 */
export const fileExistCache = ref(new Map<string, boolean>())
/**
 * 判断文件是否为文件夹，用于存储文件是否为文件夹状态
 */
export const isFolderCache = ref(new Map<string, boolean>())

/**
 * 显示缩略图图片
 */
export const displayThumbnailImage = ref(true)

/**
 * 检查多个文件是否存在
 * @param filePaths 文件路径列表
 */
export const checkFilesExist = (filePaths: string): void => {
  console.debug('检查多个文件是否存在:', filePaths)
  if (!filePaths) {
    return
  }
  const filePathList: Array<string> = JSON.parse(filePaths)
  if (filePathList.length === 1) {
    const filePath: string = filePathList[0]
    loadFileExist(filePath)
    loadFileIsFolder(filePath)
  } else {
    filePathList.forEach(filePath => {
      loadFileExist(filePath)
      loadFileIsFolder(filePath)
    })
  }
}

/**
 * 检查单个文件是否存在
 * @param filePath 文件路径
 */
export const checkFileExist = (filePath: string): void => {
  console.debug('检查单个文件是否存在:', filePath)
  if (!filePath) {
    return
  }
  loadFileExist(filePath)
  loadFileIsFolder(filePath)
}

/**
 * 加载文件是否存在
 * @param filePath 文件路径
 */
async function loadFileExist(filePath: string): Promise<void> {
  if (!filePath) {
    return
  }
  console.debug('文件是否存在:', filePath)
  try {
    fileExistCache.value.set(filePath, await exists(filePath))
  } catch (e) {
    console.error('检查文件是否存在失败:', e)
  }
}

/**
 * 加载文件夹是否存在
 * @param filePath 文件路径
 */
async function loadFileIsFolder(filePath: string): Promise<void> {
  if (!filePath) {
    return
  }
  const fileName = filePathConvertFileName(filePath)
  // 如果文件名包含"."，则不是文件夹
  if (fileName.includes('.')) {
    return
  }
  console.debug('文件夹是否存在:', filePath)
  try {
    isFolderCache.value.set(filePath, (await readDir(filePath)) !== null)
    console.debug('文件夹是否存在:', isFolderCache.value.get(filePath))
  } catch (e) {
    console.error('检查文件是否存在失败:', e)
  }
}

/**
 * 移除缓存
 * @param filePath 文件路径
 */
export const removeCache = (filePath: string): void => {
  fileExistCache.value.delete(filePath)
  isFolderCache.value.delete(filePath)
}

/**
 * 初始化用户设置
 */
export const initUserSettings = async (): Promise<void> => {
  await initSettings()
  await initShortcutKeys()
  info('初始化用户设置完成')
}

/**
 * 定时检查文件是否存在
 */
let fileExistTimer: any = null

function initFileExistTimer(): NodeJS.Timeout {
  return setInterval(async () => {
    fileExistCache.value.forEach((_value, key) => {
      loadFileExist(key)
    })
  }, 1000 * 60) // 时间可调整
}

/**
 * 初始化文本内容更新监听
 */
let updateDisplayThumbnailImageListener: any = null

async function initUpdateDisplayThumbnailImageListener(): Promise<UnlistenFn> {
  return await listen('update-display-thumbnail-image', async (event: any) => {
    displayThumbnailImage.value = event.payload.displayThumbnailImage
  })
}

/**
 * 初始化文件数据配置
 */
export async function initializeFileData(): Promise<void> {
  // 初始化文件是否存在定时任务
  fileExistTimer = initFileExistTimer()
  // 添加更新数据是否显示缩略图事件监听
  updateDisplayThumbnailImageListener = await initUpdateDisplayThumbnailImageListener()
}

/**
 * 销毁文件数据配置
 */
export async function destroyFileData(): Promise<void> {
  // 清除文件是否存在定时任务
  if (fileExistTimer) {
    clearInterval(fileExistTimer)
  }
  // 销毁监听修改数据是否显示缩略图事件
  if (updateDisplayThumbnailImageListener) {
    updateDisplayThumbnailImageListener()
  }
}
