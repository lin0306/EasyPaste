...
export const initUpdateAutoCheckUpdateListener = async (): Promise<UnlistenFn> => {
 ...
 * 初始化更新器配置
 */
export const initializeUpdater = async (): Promise<void> => {
  try {
    // 初始化修改自动检查更新任务状态事件监听
    autoCheckUpdateUnListener = await initUpdateAutoCheckUpdateListener()

    // 开启自动检查更新
    getAutoCheckUpdate().then(isAutoUpdate => {
      if (isAutoUpdate) {
        getUpdateMode().then(async updateMode => {
          const update = UpdaterService.getInstance()
          ...
          // 启动时检查一次
          update.checkForUpdates(false).catch(e => {
            error('检查更新出现异常' + e)
          })
        })
      }
 ...
 * 销毁更新器配置
 */
export const destroyUpdater = (): void => {
  // 清除修改自动检查更新任务状态事件监听
