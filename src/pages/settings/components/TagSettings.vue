...
const bindTagBtnShowLocation = computed(() => [
  {
    value: SETTINGS.TAG.BIND_TAG_LOCATION.TOP_RIGHT,
    label: currentLanguage.value.pages.settings.topRight,
  },
  {
    value: SETTINGS.TAG.BIND_TAG_LOCATION.BOTTOM_RIGHT,
    label: currentLanguage.value.pages.settings.bottomRight,
  },
])

// 标签列表展示位置
const tagListLocation = computed(() => [
  {
    value: SETTINGS.TAG.TAG_LIST_LOCATION.TOP_LEFT,
    label: currentLanguage.value.pages.settings.topLeft,
  },
  {
    value: SETTINGS.TAG.TAG_LIST_LOCATION.BOTTOM_LEFT,
    label: currentLanguage.value.pages.settings.bottomLeft,
  },
])
 ...
 */
const onChangeEnableTag = async (enableTag: boolean): Promise<void> => {
 ...
 */
const onChangeBindTagBtnShowLocation = async (bindTagBtnShowLocation: string): Promise<void> => {
...
/**
 * 修改标签列表显示位置
 * @param tagListLocation 标签列表显示位置
 */
const onChangeTagListLocation = async (tagListLocation: string): Promise<void> => {
  if (tagListLocation === originalConfig.tagListLocation) {
    return
  }
  onLoading.value = true
  try {
    await saveTagListLocation(tagListLocation)
    // 发送更新了启用标签状态消息
    await emit('update-tag-list-location', {
      location: currentConfig.tagListLocation,
    })
    originalConfig.tagListLocation = tagListLocation
    currentConfig.tagListLocation = tagListLocation
  } catch (e) {
    error('修改标签列表显示位置设置出错:' + e)
    message.error(currentLanguage.value.pages.settings.saveFailedMsg)
    currentConfig.tagListLocation = originalConfig.tagListLocation
  } finally {
    onLoading.value = false
  }
}
...
onMounted(async () => {
  try {
    ...
    const tagListLocation = await getTagListLocation()
    originalConfig.tagListLocation = tagListLocation
    currentConfig.tagListLocation = tagListLocation
  } catch (e) {
...
<template>
  <div class="settings-section">
    <n-divider title-placement="left">{{ currentLanguage.pages.settings.tagTitle }}</n-divider>
    <div class="form-item">
      <span class="label">{{ currentLanguage.pages.settings.enableTag }}</span>
      <n-switch
        v-model:value="currentConfig.enableTag"
        :loading="onLoading"
        :disabled="onLoading"
    ...
    <div class="form-item" v-if="currentConfig.enableTag">
      <span class="label">{{ currentLanguage.pages.settings.tagListLocation }}</span>
      <n-select
        class="select"
        v-model:value="currentConfig.tagListLocation"
        :options="tagListLocation"
        :loading="onLoading"
        :disabled="onLoading"
        @update:value="onChangeTagListLocation"
      />
    </div>
  </div>
</template>
