 ...
 * @param key 快捷键类型
 */
const startEditShortcut = (key: keyof ShortcutKeys): void => {
  ...
  if (key.toString() === 'wakeUpRoutine') {
    getWakeUpRoutineKeyAvailable().then(available => {
      ...
      currentHintText.value = currentKeyAvailableKey.value
        ? currentLanguage.value.pages.settings.keyAvailableHint
        : currentLanguage.value.pages.settings.keyNotAvailableHint
    ...
    if (failedNumbers.length > 0) {
      ...
      currentHintText.value = currentKeyAvailableKey.value
        ? currentLanguage.value.pages.settings.keyAvailableHint
        : currentLanguage.value.pages.settings.quickPasteKeyNotAvailableHint.replace(
            '{keys}',
            failedNumbers
              .map(i => formatKeysDisplay([...currentShortcutKeys[key]?.key, String(i)]))
              .join(',')
          )
      initialHintText.value = currentHintText.value
    }
 ...
 * 确认编辑快捷键
 */
const confirmEditShortcut = async (): Promise<void> => {
  try {
...
const handleKeyDown = async (event: any): Promise<void> => {
  ...
  if (editingShortcut.value && editingShortcut.value.toString() === 'wakeUpRoutine') {
    const isRepeat =
      JSON.stringify(tempKeys.value) === JSON.stringify(currentShortcutKeys.search.key)
    // 快捷键和搜索快捷键冲突
    if (isRepeat) {
      currentKeyAvailableKey.value = false
      currentHintText.value = currentLanguage.value.pages.settings.keyRepeatHint.replace(
        '{keys}',
        currentLanguage.value.pages.settings.search
      )
    } else {
  ...
  if (editingShortcut.value && editingShortcut.value.toString() === 'search') {
    const isRepeat =
      JSON.stringify(tempKeys.value) === JSON.stringify(currentShortcutKeys.wakeUpRoutine.key)
    // 快捷键和唤醒程序快捷键冲突
    if (isRepeat) {
      currentKeyAvailableKey.value = false
      currentHintText.value = currentLanguage.value.pages.settings.keyRepeatHint.replace(
        '{keys}',
        currentLanguage.value.pages.settings.wakeUpRoutine
      )
    }
  }
 ...
 * 检查打开主窗口快捷键是否可用
 */
const checkWakeUpRoutineKeys = async (): Promise<void> => {
  const key = editingShortcut.value || ''
 ...
 * 检查快捷粘贴快捷键是否可用
 */
const checkQuickPasteKeys = async (): Promise<void> => {
  const key = editingShortcut.value || ''
  const isUpdate = JSON.stringify(currentShortcutKeys[key].key) !== JSON.stringify(tempKeys.value)
  if (isUpdate) {
    const failedNumbers: number[] = []
    ...
    currentHintText.value = currentKeyAvailableKey.value
      ...
      : currentLanguage.value.pages.settings.quickPasteKeyNotAvailableHint.replace(
          '{keys}',
          failedNumbers.map(i => formatKeysDisplay([...tempKeys.value, String(i)])).join(',')
        )
  } else {
    currentKeyAvailableKey.value = initialKeyAvailableKey.value
    currentHintText.value = initialHintText.value
...
<template>
  <div class="settings-section">
    ...
    <n-modal
      ...
      :title="currentLanguage.pages.settings.editHotkeyModalTitle"
      preset="dialog"
    >
