 ...
 */
const onChangeAutoCheckUpdate = async (autoCheckUpdate: boolean): Promise<void> => {
 ...
 */
const onChangeUpdateMode = async (updateMode: string): Promise<void> => {
  if (updateMode === originalConfig.updateMode) {
    return
  }
  onLoading.value = true
  try {
 ...
 * 修改自动检查更新间隔
 */
const onChangeAutoCheckUpdateInterval = async (): Promise<void> => {
 ...
 */
const onChangeNewVersionAlertMode = async (newVersionAlertMode: string): Promise<void> => {
  if (newVersionAlertMode === originalConfig.newVersionAlertMode) {
    return
  }
  onLoading.value = true
  try {
...
<template>
  <div class="settings-section">
    <n-divider title-placement="left">{{ currentLanguage.pages.settings.updaterTitle }}</n-divider>
    <div class="form-item">
      <span class="label">{{ currentLanguage.pages.settings.autoCheckUpdate }}</span>
      <n-switch
        v-model:value="currentConfig.autoCheckUpdate"
        :loading="onLoading"
        :disabled="onLoading"
    ...
    <div
      class="line"
      v-if="
        currentConfig.autoCheckUpdate &&
        currentConfig.updateMode === SETTINGS.UPDATER.CHECK_MODE.TIMING
      "
    >
      <div class="main-item">
