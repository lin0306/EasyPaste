...
const isLoadingLogFile = ref(true)

/**
 * 修改最大历史记录项数
 */
const onChangeMaxHistoryItems = async (): Promise<void> => {
 ...
 * 修改数据保留天数
 */
const onChangeDataRetentionDays = async (): Promise<void> => {
 ...
 * 修改是否保存图片
 */
const onChangeEnableImageSave = async (): Promise<void> => {
 ...
 * 更新图片存储路径
 */
const handleUpdateImagePath = async (): Promise<void> => {
  ...
  if (items && items.length > 0) {
    ...
    for (let item of items) {
      // 获取旧文件的文件夹路径
      const oldBasePath = item.file_path.substring(
        0,
        item.file_path.lastIndexOf(isMac ? '/' : '\\')
      )
 ...
 * 获取日志文件信息
 */
const loadLogFileInfo = async (): Promise<void> => {
  isLoadingLogFile.value = true
  logFileInfo.path = (await appLocalDataDir()) + (isMac ? '/' : '\\') + 'logs'
  const entries = await readDir(logFileInfo.path)
  let fileSize = 0
  if (entries && entries.length > 0) {
    for (const file of entries) {
      const info = await stat(logFileInfo.path + (isMac ? '/' : '\\') + file.name)
      fileSize += info.size
    }
  }
  logFileInfo.size = fileSize
  isLoadingLogFile.value = false
}
...
const openLogFolder = async (): Promise<void> => {
  await invoke('open_folder', { path: logFileInfo.path })
}
...
const clearLogFiles = async (): Promise<void> => {
  ...
  try {
    ...
    if (entries && entries.length > 0) {
      for (const file of entries) {
        deleteFile(logFileInfo.path + (isMac ? '/' : '\\') + file.name)
      }
    }
    await loadLogFileInfo()
    message.success(currentLanguage.value.pages.settings.logClearSuccessMsg)
  } catch (e) {
    error('清空日志文件失败:' + e)
  } finally {
    onLoading.value = false
  ...
  <div class="settings-section">
    ...
    <div class="line">
      <div class="main-item">
        <span class="label">
          {{
            currentLanguage.pages.settings.logStorageUsage +
    ...
    <n-divider title-placement="left">{{
      currentLanguage.pages.settings.imageStorageTitle
    }}</n-divider>
    <div class="line">
      <div class="main-item">
        <span class="label">{{ currentLanguage.pages.settings.enableImageSave }}</span>
        <n-switch
          v-model:value="currentConfig.enableImageSave"
          :loading="onLoading"
          :disabled="onLoading"
    ...
    <div class="input-item" v-if="currentConfig.enableImageSave">
    ...
    <!-- 更新文件地址弹窗 -->
    <n-modal
      v-model:show="updateImagePathVisible"
      :title="currentLanguage.pages.settings.replaceImageStorageLocationTitle"
      preset="dialog"
    >
      <p v-if="!isUpdateImagePath">
        {{
          currentLanguage.pages.settings.replaceImageStorageLocationConfirm.replace(
            '{num}',
            String(replaceImageItemCount)
          )
        }}
      </p>
      <div v-else class="replace-image-storage-location-progress">
        ...
        <div class="replace-image-storage-location-stat">
          {{ moveImageCount }} / {{ replaceImageItemCount }}
        </div>
      </div>
