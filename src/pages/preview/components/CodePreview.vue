 ...
 * 更新忽略大小写，重新进行筛选
 */
const updateIgnoreCaseAndSearch = (): void => {
      ...
      if (regExp.test(textContent)) {
        ...
        parts.forEach(part => {
          ...
          if (part.toLowerCase() === searchText.value.toLowerCase()) {
            ...
            span.className =
              cycleIndex === currentMatchIndex.value ? 'current-match' : 'other-match'
            cycleIndex++
...
// 恢复原始高亮
const restoreOriginalHighlight = (): void => {
...
// 滚动到当前匹配项
const scrollToCurrentMatch = (): void => {
...
const prevMatch = (): void => {
  ...
  currentMatchIndex.value =
    (currentMatchIndex.value - 1 + matchQuantity.value) % matchQuantity.value
  highlightMatches()
  scrollToCurrentMatch()
}
...
.code-content {
  ...
  height: 100%;
  overflow: auto;
  border: 3px solid var(--theme-universal-border);
  border-radius: 5px;
}
