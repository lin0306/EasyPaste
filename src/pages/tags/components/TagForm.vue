...
const addTag = async (): Promise<void> => {
  ...
  try {
    ...
    message.error(
      editState.isEdit
        ? currentLanguage.value.pages.tags.editFailedMsg
        : currentLanguage.value.pages.tags.saveFailedMsg
    )
  }
}
</script>
  ...
  <div class="tag-detail-panel" :class="{ 'panel-visible': isPanelVisible }">
    <div class="panel-header">
      <h3>
        {{
          editState.isEdit
            ? currentLanguage.pages.tags.editTitle
            : currentLanguage.pages.tags.addTitle
        }}
      </h3>
      <div>
        <font-awesome-icon
    ...
    <div class="panel-content">
      <div class="tag-form">
        <div class="form-item">
          ...
          <n-input
            v-model:value="editState.tagName"
        ...
        <div class="form-item">
          ...
          <div class="preview-tag-item" :style="{ backgroundColor: editState.tagColor }">
            <span
              class="preview-tag-name"
              :style="{ color: getContrastColor(editState.tagColor) }"
              >{{ editState.tagName }}</span
            >
          </div>
          <n-tag size="small" round closable bordered class="preview-item-tag">
            <div class="preview-item-tag-content">
              <div
                :style="{ backgroundColor: editState.tagColor }"
                class="preview-item-tag-color"
              ></div>
