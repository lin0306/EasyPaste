...
// 是否已显示开发者工具
const isShowDevTool = ref(false);

// 修改开发工具的显示状态
async function updateDevtoolConfigShowStatus(checked: boolean) {
    info('修改开发工具的显示状态:' + checked);
    if (checked) {
        showDevToolListen.show();
    } else {
        showDevToolListen.hide();
    }
    await emit("update-show-dev-tool", checked);
}
...
onMounted(async () => {
    ...
    isShowDevTool.value = await showDevToolListen.isShow();
});
</script>
<template>
  ...
  <n-switch v-model:value="currentConfig.replaceGlobalHotkey" />
                    ...
                    <div class="form-item">
                        <span class="label">{{ currentLanguage.pages.settings.devTools }}</span>
                        <n-switch v-model:value="isShowDevTool" @update:value="updateDevtoolConfigShowStatus" />
                    </div>
                </div>
