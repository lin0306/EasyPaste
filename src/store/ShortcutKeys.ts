 ...
 * @param keys 快捷键
 */
export async function saveSearchKey(keys: any): Promise<void> {
  info('保存搜索快捷键' + JSON.stringify(keys))
  const settings = await load(SHORTCUT_KEYS_FILE_NAME, { defaults: {}, autoSave: true })
...
export async function getSearchKey(): Promise<ShortcutKeys['search']> {
  const settings = await load(SHORTCUT_KEYS_FILE_NAME, { defaults: {}, autoSave: true })
  return (
    (await settings.get<ShortcutKeys['search']>(SHORTCUT_KEYS_KEYS.SEARCH)) ||
    defaultShortcutKeys.search
  )
}
 ...
 * @param keys 快捷键
 */
export async function saveWakeUpRoutineKey(keys: ShortcutKey): Promise<void> {
...
export async function getWakeUpRoutine(): Promise<ShortcutKeys['wakeUpRoutine']> {
  const settings = await load(SHORTCUT_KEYS_FILE_NAME, { defaults: {}, autoSave: true })
  return (
    (await settings.get<ShortcutKeys['wakeUpRoutine']>(SHORTCUT_KEYS_KEYS.WAKE_UP_ROUTINE)) ||
    defaultShortcutKeys.wakeUpRoutine
  )
}
 ...
 * @param keys 快捷键
 */
export async function saveQuickPasteKey(keys: ShortcutKey): Promise<void> {
  info('保存快速粘贴快捷键' + JSON.stringify(keys))
  const settings = await load(SHORTCUT_KEYS_FILE_NAME, { defaults: {}, autoSave: true })
  await settings.set(SHORTCUT_KEYS_KEYS.QUICK_PASTE, keys)
}

/**
 * 获取快速粘贴快捷键
 */
export async function getQuickPaste(): Promise<ShortcutKeys['quickPaste']> {
  const settings = await load(SHORTCUT_KEYS_FILE_NAME, { defaults: {}, autoSave: true })
  return (
    (await settings.get<ShortcutKeys['quickPaste']>(SHORTCUT_KEYS_KEYS.QUICK_PASTE)) ||
    defaultShortcutKeys.quickPaste
  )
}
 ...
 * 初始化用户快捷键配置
 */
export async function initShortcutKeys(): Promise<void> {
  info('初始化用户快捷键配置')
  ...
  if (shortcutKeysExist) {
    ...
    if (!shortcutKeysString.includes(SHORTCUT_KEYS_KEYS.QUICK_PASTE)) {
