...
- **macOS 用户**: 下载 `.tar.gz` 文件，解压后将应用拖入应用程序文件夹
- **国内用户**: 如果 GitHub 访问缓慢，建议使用国内加速源
- **首次安装**: 首次启动时会自动初始化数据库和配置文件

### 🔐 安全验证

所有发布版本都经过数字签名验证，确保安全可靠：
- Windows 安装程序已签名
- macOS 应用已公证
- 所有版本都包含签名文件供验证
...
| 技术               | 版本      | 用途                         |
|------------------|---------|----------------------------|
| **Vue 3**        | 3.5.16  | 使用组合式 API 构建响应式用户界面        |
| **TypeScript**   | 5.6.3   | 类型安全的 JavaScript 超集，启用严格模式 |
| **Naive UI**     | 2.44.1  | 基于 Vue 3 的现代化 UI 组件库       |
| **Vue Router**   | 4.5.1   | 官方路由管理器，支持嵌套路由和导航守卫        |
| **Pinia**        | 3.0.3   | Vue 3 官方状态管理库，替代 Vuex      |
| **Vite**         | 8.0.13  | 下一代前端构建工具，支持 HMR 和 ESM     |
| **Highlight.js** | 11.11.1 | 多语言代码语法高亮库                 |
| **Marked**       | 16.2.1  | 高性能 Markdown 解析器和编译器       |
| **FontAwesome**  | 7.2.0   | 图标字体库，提供丰富的矢量图标            |
| **Lodash-ES**    | 4.18.0  | JavaScript 实用工具库，ES 模块版本   |
| **GSAP**         | 3.15.0  | 缓动动画库，用于创建平滑的动画效果          |

### 后端技术栈

| 技术               | 版本      | 用途                        |
|------------------|---------|---------------------------|
| **Tauri**        | 2.11.2  | 基于 Rust 的跨平台桌面应用框架        |
| **Rust**         | 1.95.0+ | 系统级编程语言，内存安全且高性能          |
| **SQLite**       | 最新版     | 轻量级嵌入式关系数据库               |
| **clipboard-rs** | 0.3.4   | 跨平台剪贴板操作库                 |
| **serde**        | 1.0.228 | Rust 序列化/反序列化框架           |
| **chrono**       | 0.4.44  | Rust 日期时间处理库              |
| **dirs**         | 6.0.0   | 跨平台系统目录获取库                |
| **lazy_static**  | 1.5.0   | Rust 静态变量延迟初始化            |
| **reqwest**      | 0.12.28 | Rust HTTP 客户端库            |
| **winreg**       | 0.56.0  | Windows 注册表操作库（仅 Windows） |
| **dotenv**       | 0.15.0  | 环境变量处理库                   |
...
| 依赖包                        | 版本      | 用途                  |
|----------------------------|---------|---------------------|
| `@vitejs/plugin-vue`       | ^6.0.0  | Vite Vue 单文件组件支持    |
...
| 插件                               | 版本      | 功能描述           |
|----------------------------------|---------|----------------|
| `tauri-plugin-sql`               | ^2.4.0  | SQLite 数据库操作支持 |
...
| `tauri-plugin-updater`           | ~2.10.1 | 应用自动更新机制       |
| `tauri-plugin-store`             | ~2.4.3  | 持久化配置数据存储      |
| `tauri-plugin-opener`            | ^2.5.4  | 系统默认程序打开文件     |
| `tauri-plugin-process`           | ~2.5.4  | 应用进程生命周期管理     |
| `tauri-plugin-shell`             | ~2.3.5  | Shell 命令执行接口   |
| `tauri-plugin-log`               | ~2.8.0  | 结构化日志记录系统      |
| `tauri-plugin-fs`                | ~2.5.1  | 文件系统安全操作       |
| `tauri-plugin-os`                | ~2.3.2  | 操作系统信息获取       |
| `tauri-plugin-dialog`            | ~2.7.1  | 系统对话框接口        |
| `tauri-plugin-http`              | ~2.5.9  | HTTP 请求支持      |
...
| 依赖包            | 版本       | 用途描述                    |
|----------------|----------|-------------------------|
| `tauri`        | 2.11.2   | Tauri 核心框架，支持托盘和 PNG 图像 |
| `clipboard-rs` | 0.3.4    | 跨平台剪贴板内容读写操作            |
| `serde`        | 1.0.228  | Rust 序列化/反序列化框架         |
| `serde_json`   | 1.0.150  | JSON 格式数据处理             |
| `chrono`       | 0.4.44   | 日期时间解析和格式化              |
| `dirs`         | 6.0.0    | 跨平台系统目录路径获取             |
| `log`          | 0.4.30   | 结构化日志记录接口               |
| `lazy_static`  | 1.5.0    | 编译时静态变量初始化              |
| `reqwest`      | 0.13.3   | HTTP 客户端库，用于网络请求        |
| `dotenv`       | 0.15.0   | 环境变量读取和设置               |
...
| 依赖包      | 版本     | 平台      | 用途描述            |
|----------|--------|---------|-----------------|
| `winreg` | 0.56.0 | Windows | Windows 注册表读写操作 |
...
| `tar`    | 0.4.46 | TAR 归档文件处理        |
| `flate2` | 1.1.9  | GZIP/DEFLATE 压缩算法 |
...
| 依赖包           | 版本    | 用途描述           |
|---------------|-------|----------------|
| `tauri-build` | 2.6.2 | Tauri 应用构建脚本支持 |
...
│   │   └── 📁 updater/                     # 更新页面
│   ├── 📁 routers/                         # 路由配置
│   ├── 📁 services/                        # 业务服务层 (9个服务文件)
│   ├── 📁 store/                           # Pinia 状态管理 (7个store文件)
│   ├── 📁 types/                           # TypeScript 类型定义 (15+个类型文件)
│   ├── 📁 utils/                           # 工具函数 (17个工具文件)
│   ├── 📄 App.vue                          # 根组件
│   └── 📄 main.ts                          # 应用入口
│
├── 📁 src-tauri/                           # 后端源代码
│   ├── 📁 src/
│   │   ├── 📁 commands/                    # Tauri 命令处理 (6个模块)
│   │   ├── 📁 listener/                    # 剪贴板监听服务 (5个模块)
│   │   ├── 📁 i18n/                        # 国际化支持 (8个模块)
│   │   ├── 📁 tray/                        # 系统托盘 (5个模块)
│   │   ├── 📁 windows/                     # 窗口管理 (4个模块)
│   │   ├── 📁 log/                         # 日志配置 (3个模块)
...
- 链接（支持展示链接网页标题）
- 文件路径（不复制文件本身，只记录路径）
- 使用第三方软件复制的图片（由于无法获取图片所在位置/图片是临时文件，需存储图片元数据）
- 多种文件类型预览：图片、视频、音频、压缩包等
...
| [Highlight.js](https://highlightjs.org/)     | 代码高亮库        | BSD     | 提供了丰富的代码高亮样式      |
| [Fort Awesome](https://fontawesome.com/)     | 字体图标库        | MIT     | 提供了丰富的图标库         |
| [GSAP](https://gsap.com/)                    | 动画库          | 标准免费许可证 | 提供了丰富的动画效果        |
