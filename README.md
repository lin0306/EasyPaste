...
[![](https://img.shields.io/badge/GitHub%20version-v0.1.6--beta-blue.svg?cacheSeconds=2592000)](https://github.com/lin0306/EasyPaste/releases/latest)
[![](https://img.shields.io/badge/Gitee%20version-v0.1.6--beta-green.svg?cacheSeconds=2592000)](https://gitee.com/lin0306/easy-paste/releases/latest)
...
| 技术 | 用途 |
|------|------|
| **Vue 3.5.13** | 使用组合式 API 构建用户界面 |
| **TypeScript 5.6.2** | 提供类型安全的代码编写体验，严格模式 |
| **Naive UI 2.41.0** | 美观现代的 UI 组件库 |
| **Vue Router 4** | 页面路由管理 |
| **Pinia 3.0.2** | 状态管理库 |
| **Vite 6.0.3** | 现代前端构建工具，支持 HMR |

### 后端技术栈
| 技术                     | 用途 |
|------------------------|------|
| **Tauri 2.7.0**        | 使用 Rust 构建的跨平台应用框架 |
| **Rust 1.86.0**        | 高性能、内存安全的系统编程语言 |
...
| `vfonts ^0.0.3` | 字体支持          |
| `@highlightjs/vue-plugin ^2.1.0` | 代码高亮支持        |
| `highlight.js ^11.11.1` | 语法高亮库         |
| `markdown-it ^14.1.0` | Markdown格式支持  |
...
| 插件 | 功能 |
|------|------|
| `tauri-plugin-sql ^2.2.0` | SQLite 数据库支持 |
| `tauri-plugin-global-shortcut ~2` | 全局快捷键支持 |
| `tauri-plugin-autostart ~2` | 开机自启动 |
| `tauri-plugin-notification ~2.3.0` | 系统通知 |
| `tauri-plugin-updater ~2` | 应用自动更新 |
| `tauri-plugin-store ~2` | 配置数据存储 |
| `tauri-plugin-opener ^2` | 文件打开 |
| `tauri-plugin-process ~2` | 进程管理 |
| `tauri-plugin-shell ~2` | Shell 命令执行 |
| `tauri-plugin-log ~2` | 日志管理 |
| `tauri-plugin-clipboard-manager ~2` | 剪贴板管理 |
| `tauri-plugin-fs ~2` | 文件系统操作 |
| `tauri-plugin-window-state ~2` | 窗口状态管理 |
...
| 依赖包 | 用途            |
|--------|---------------|
| `clipboard-rs 0.3.0` | 跨平台剪贴板操作      |
| `serde & serde_json 1` | JSON 序列化/反序列化 |
| `chrono 0.4.19` | 日期时间处理        |
| `dirs 6.0.0` | 系统目录获取        |
| `log 0.4` | 日志记录          |
| `lazy_static 1.5.0` | 静态变量管理        |
| `winreg 0.55.0` | Windows注册表操作  |
...
│   │   ├── CopyState.ts       # 复制状态类型
│   │   ├── language.ts        # 语言类型
│   │   ├── NavBarItem.ts      # 导航栏项目类型
│   │   ├── Themes.ts          # 主题类型
│   │   ├── UpdaterInfo.ts     # 更新信息类型
│   │   └── Window.ts          # 窗口类型
│   ├── 📁 utils/              # 工具函数
