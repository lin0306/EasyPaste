...
pub fn init<R: Runtime>() -> TauriPlugin<R> {
    Builder::new("clipboard_manager")
        .setup(|app, _| {
            // 初始化剪贴板监听状态
            app.manage(ClipboardMonitorState::new(true));
...
fn start_clipboard_monitor<R: Runtime>(app: tauri::AppHandle<R>) {
    ...
    thread::spawn(move || {
        ...
        let state = app.state::<ClipboardMonitorState>();
...
// 添加新的命令
#[tauri::command]
pub fn set_clipboard_monitor_enabled<R: Runtime>(app: AppHandle<R>, enabled: bool) {
    let state = app.state::<ClipboardMonitorState>();
    state.set_enabled(enabled);
    info!("剪贴板监听状态已更新: {}", enabled);
}

#[tauri::command]
pub fn is_clipboard_monitor_enabled<R: Runtime>(app: AppHandle<R>) -> bool {
    let state = app.state::<ClipboardMonitorState>();
    state.is_enabled()
}
